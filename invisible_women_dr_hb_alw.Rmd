...
To go one step deeper into the comparison of the gendered vs nongendered datasets we attempted to do a topicmodelling for both subsets using the descriptions of the datasets. Using the LDA algorithm, we extracted 10 topics for each subset. The applicability of the LDA algorithm to our case is limited, as the quantity of words in the description of the datasets is rather small. In our case that led to the situation that the unsupervised algorithm produced a variation of topics while re-running it. Therefore the results have to be interpreted with caution.
