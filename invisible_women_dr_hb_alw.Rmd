...
Unsurprisingly, the data sets that are connected to women have a caring connotation. Important to note is, that one dataset can be assigned to several topics. 

So, in the next step we wanted to find out what gendered vs. non-gendered datasets in these broad topics are about. We did that with the help of the LDA algorithm. This algorithm (it is like an elaborate instruction for the computer on what to do) - the LDA (latent dirichlet allocation is a generative statistical model that allows sets of observations to be explained by grouping them through computation) - can be used to find the most important groups (aka topics) of words. Thus, the name: topic modeling. 

- We used the descriptions of the datasets as input to feed the algorithm as they provided the biggest quantity of text 
- we did that for the four topics that contain most gender
- realized that there is only limited applicability, unsupervised algorithm, missing reprudocability, still a bit unleaned data
- therefore limited liability of the results - we only show you a selection for the health topic field
- 
#and hand-selected three of the 10 topics that the algorithm spit out. Once for the subset of not gendered health datasets and once for the subset of health dataset that contain gender.
